    ...
    def generate_markdown(self, result: EvaluationResult) -> str:
        ...
        lines.extend(
            [
                ...
                "## Cost Analysis",
                "",
                f"- **Total Cost**: ${result.overall.total_cost:.6f} USD",
                f"- **Tokens**: {result.overall.input_tokens:,} input / {result.overall.output_tokens:,} output",
                f"- **Cost per Quality Point**: ${result.overall.cost_per_quality:.6f}",
                f"- **Efficiency Rating**: {result.overall.efficiency_rating}",
                "",
                "### Cost Efficiency Thresholds",
                "",
                "| Rating | Cost per Point | Description |",
                "|--------|----------------|-------------|",
                "| Excellent | < $0.0005 | Very efficient |",
                "| Good | < $0.002 | Typical DeepSeek |",
                "| Fair | < $0.005 | Higher usage |",
                "| Poor | >= $0.005 | Expensive |",
                "",
                "## Phase Details",
                "",
            ]
