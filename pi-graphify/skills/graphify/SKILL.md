...
## pi / MCP integration (sage workflow)

The `pi-graphify` pi extension (in this same repo at `pi-graphify/`) wraps graphify as an MCP server via `pi-mcp-adapter`. After running the build phase above once, you can query the graph interactively from pi using first-class `mcp_graph_*` tools (lazy MCP start, ~1s cold start on first call).

### When to use MCP tools vs re-running graphify

| You want to… | Use |
|---|---|
| Build the graph for the first time / after big changes | `bash({ command: "graphify ." })` (~5-10 min) |
| Natural-language question about the corpus | `mcp_graph_query({ question: "..." })` |
| Path between two known nodes | `mcp_graph_shortest_path({ source, target })` |
| Metadata on a specific node | `mcp_graph_get_node({ label })` |
| Direct neighbors of a node | `mcp_graph_get_neighbors({ node, depth })` |
| Community containing a node | `mcp_graph_get_community({ node })` |
| Top-N nodes by centrality | `mcp_graph_god_nodes({ limit })` |
| Graph overview (size, density, communities) | `mcp_graph_graph_stats({})` |

### Decision tree

```
Need information from the graph?
├── Already have node labels / symbols in mind?
│   ├── YES → mcp_graph_get_node or mcp_graph_get_neighbors
│   └── NO  → Natural-language question?
│       ├── YES → mcp_graph_query  (semantic over graph)
...
### Complementary tools (NOT graphify)

- **Code editing** → use `serena_replace_symbol_body` (pi-serena extension)
- **AST graph / git impact** → use `mcp_trace_path` / `mcp_detect_changes` (pi-codebase-memory extension)
