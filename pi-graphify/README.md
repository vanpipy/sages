...
> ⚠️ **v0.1.0**: Knowledge graph queries via MCP. 7 first-class tools (query/path/explain/god_nodes/etc.) + proxy. Handles mixed corpus (code + docs + papers + videos + images) — **complement** to codebase-memory-mcp, not replace it.
...
3. Ships `skills/graphify/SKILL.md` so the LLM knows which tool to pick
...
└── templates/
    └── mcp.json              # graphify MCP server config
```
