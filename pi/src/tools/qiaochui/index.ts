...
export function registerQiaoChuiTools(pi: ExtensionAPI): void {
   ...
   *     The LLM reads the draft using read / graphify_query,
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      try {
        ...
        const intent = scope
          ? `Scope-driven design detected (tier '${scope.tier}', in-scope: ${scope.inScope.join(", ")}). Read draft.md with the read tool; the heuristic below scores only in-scope planes. Then call qiaochui_review with observation {score, notes?} using the 6-dimension scope-aware rubric.`
          : `Read draft.md using semantic tools (read, graphify_query) to assess the 5 dimensions below. Then call qiaochui_review with observation {score, notes?} to persist the score.`;
