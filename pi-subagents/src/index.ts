...
export function renderRunningAgentStatus(
	...
	container.addChild(
		new Text(
			theme.fg("accent", frame) + (statsText ? " " + statsText : ""),
			0,
			0,
		),
	);
					...
					Type.Union(
						[
							Type.Literal("worktree", {
								description:
									"Legacy alias for the explicit worktree object — Sages callers MUST pass " +
									"an explicit worktree object instead. The literal is accepted by the schema " +
							...
							Type.Object(
								{
									...
									worktree_id: Type.Optional(
										Type.String({
											description:
												"Optional sub-id within a task. Defaults to task_id when omitted.",
											pattern: "^[A-Za-z0-9_-]+$",
										}),
									),
									mode: Type.Union(
										[Type.Literal("create"), Type.Literal("reuse")],
										{
											description:
			...
			execute: async (toolCallId, params, signal, onUpdate, ctx) => {
				...
				if (isScopeModelsEnabled() && model) {
					...
					if (allowed && !isModelInScope(model, allowed)) {
						...
						const modelLabel =
							resolvedConfig.modelInput ?? `${model.provider}/${model.id}`;
						ctx.ui.notify(
				...
				// The resolved isolation may be either the legacy "worktree" literal
				// (carried through) or a parsed managed-worktree request. We forward
				// the request through `managedWorktree` and only pass the literal here.
				const isolationRaw = resolvedConfig.isolation;
				const isolation: IsolationMode | undefined =
					typeof isolationRaw === "string" ? isolationRaw : undefined;
				const managedWorktree = resolvedConfig.managedWorktree;

				// GC-2026-008 P2: refuse the legacy "worktree" string literal BEFORE
				// child execution. Sages callers MUST supply the explicit object so
				// they cannot silently fall back to the /tmp-backed ephemeral worktree.
				// We reject by checking the raw value (resolvedConfig.isolation is
				// already coerced through the parser, so the literal gets surfaced as
				// `isolation === "worktree"` with no managedWorktree attached).
				if (params.isolation === "worktree" && managedWorktree === undefined) {
					return textResult(
				...
				const agentInvocation: AgentInvocation = {
					...
					inheritContext,
					runInBackground,
					// For the legacy `isolation: "worktree"` literal we reject BEFORE
					// reaching here (see the early `textResult` above); for the object
					// form we surface the resolved isolation (still effectively
					// "worktree" semantically) so existing UI tags keep working. The
					// actual managed-worktree placement happens through `managedWorktree`.
					isolation: typeof isolation === "string" ? "worktree" : undefined,
				};
				...
				if (params.schedule) {
					...
					try {
						...
						// Note: the scheduler path inherits the caller's resolved isolation;
						// if isolation was an object, we forward the JSON-shaped value through
						// ScheduledSubagent (it accepts either form via the existing type).
				...
				return {
					...
					handleInput: (data: string) => {
						...
						if (
							...
							NUMERIC_IDS.has(items[currentIndex].id)
						) {
