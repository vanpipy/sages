...
export interface AgentDetails {
	...
	error?: string;
	/**
	 * Structured handoff from the managed-worktree domain (GC-2026-008 P2):
	 * path / branch / baseSha / baseRef / head / dirty / leaseToken. Renderers
	 * and downstream consumers (e.g. a future `sages_release_worktree` tool)
	 * read this instead of the legacy `record.worktree` (which lives on the
	 * /tmp-backed ephemeral path).
	 */
	worktree?: ManagedWorktreeHandoff;
}

// ---- Formatting helpers ----

/** Apply foreground styling while restoring it after nested foreground/full ANSI resets. */
export function fgPreservingNestedStyles(
	theme: Theme,
	color: string,
	text: string,
): string {
