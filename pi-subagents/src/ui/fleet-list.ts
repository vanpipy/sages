...
export class FleetList {
	...
	/**
	 * GC-2026-021 B-fix: gate the 200ms re-render timer on actual work.
	 *
	 * Original behavior: `ensureTimer()` unconditionally started a
	 * 5 Hz `setInterval`, even when the agent roster was empty. Idle
	 * fleet list = 5 fires/s of no-op.
	 *
	 * New behavior: only start the timer when the roster is non-empty.
	 * When `update()` clears the roster on the next pass, the timer
	 * self-stops; new state changes re-arm it via `ensureTimer()`.
	 *
	 * Trade-off: when the roster is empty the user gets zero automatic
	 * re-renders, but they also get zero CPU cost. They can re-arm by
	 * spawning a new agent or pressing ↓ (which calls `update()`).
	 */
	ensureTimer(): void {
	...
	/** Cheap predicate: is the roster non-empty? Mirrors `update()`'s check. */
	private hasAgents(): boolean {
		if (!this.enabled) return false;
		return this.agentRecords().length > 0;
	}
	...
	update(): void {
		if (!this.ui) return;
		const hasAgents = this.hasAgents();
	...
	private openSelected(): void {
		...
		if (!record.session) {
			this.ui.notify(
				`Agent is ${record.status} — no session available.`,
				"info",
			);
			return;
		}
