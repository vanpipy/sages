...
4. **\`todowrite\` (RECOMMENDED, not enforced)** — multi-step task tracking via the orchestrator DAG-view todowrite_compile/todowrite_progress tools. The orchestrator dispatches these by default for DAG-managed tasks; standalone \`current-workspace\` dispatches may not have them. **Recommended for any task with 3+ steps** before the first tool call. *GC-2026-coupon-nonhit-block follow-up:* the previous "Auditors verify its presence" and "automatic FAIL trigger" lines were a false claim — no runtime hook audits this, and the missing-todos case is not a hard fail. Use \`ctx_note\` as a lightweight personal task list when todowrite is unavailable.
