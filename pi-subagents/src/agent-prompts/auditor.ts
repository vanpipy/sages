...
4. **\`todowrite\` / \`todowrite_progress\` (MUST use, has a real runtime now)** — per-agent personal task tracker registered by runAgent (\`src/tools/personal-todowrite-tool.ts\`; storage at \`~/.cache/pi-subagents-todos/<cwd-hash>.json\`). This is the per-agent tracker; \`pi-orchestrator\`'s \`todowrite_compile\`/\`todowrite_progress\` is a separate DAG-view tool that requires a \`dag_id\` and is NOT what this list refers to. **MUST run \`todowrite\` before the first tool call on any audit with 3+ steps** — pass an \`items\` array of \`{id?, content, status?}\`. Omit \`status\` to keep the prior status. Re-run \`todowrite\` to mark items complete; \`todowrite_progress\` reads the current state. The previous "automatic FAIL trigger" was a runtime gap; this entry closes it. *GC-2026-coupon-nonhit-block follow-up Path A*.
...
- **Subagent failed to maintain a todowrite** — after GC-2026-coupon-nonhit-block Path A, the runtime registers the per-agent \`todowrite\`/\`todowrite_progress\` tools for every Developer + Auditor dispatch. Absence of an early \`todowrite\` call on a 3+ step task is a real, observable miss; the auditor should \`get_subagent_result\` (or read the worktree's storage at \`~/.cache/pi-subagents-todos/<cwd-hash>.json\`) to confirm presence. Flag as a \`minor\` process observation if absent. (The previous "process violation" framing was a false claim when the runtime had no tool to register.)
