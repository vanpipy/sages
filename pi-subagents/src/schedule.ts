...
export interface NewJobInput {
	 ...
	 * object. Stored as-is and re-passed to `manager.spawn` at fire time.
	 */
...
export class SubagentScheduler {
	...
	private executeJob(id: string): void {
		...
		let agentId: string;
		try {
			// If the stored isolation is an explicit object, hand it to the
			// manager through the dedicated channel; if it's the legacy literal,
			// pass it through unchanged so the rejection path still fires.
			const storedIsolation = job.isolation;
			const isExplicitObject =
				typeof storedIsolation === "object" && storedIsolation !== null;
			agentId = manager.spawn(pi, ctx, job.subagent_type, job.prompt, {
				...
				isolation:
					typeof storedIsolation === "string"
						? ((storedIsolation as IsolationMode) ?? undefined)
						: undefined,
				managedWorktree: isExplicitObject
					? (storedIsolation as ParsedManagedWorktreeRequest)
					: undefined,
			});
		} catch (err) {
