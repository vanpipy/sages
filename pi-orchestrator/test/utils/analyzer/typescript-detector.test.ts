...
describe('TypeScriptDetector', () => {
  ...
  describe('canHandle', () => {
    it('should return true when package.json with TypeScript exists', () => {
      // Test with the sages pi-orchestrator workspace package (Post-PR-2 +
      // Post-GC-2026-073 — the conductor at ./pi/ was retired; the
      // orchestrator package is now the canonical TypeScript workspace).
      const result = detector.canHandle('/home/leroy/Project/sages/pi-orchestrator');
  ...
  describe('detect', () => {
    it('should detect TypeScript project with high confidence', async () => {
      const info = await detector.detect('/home/leroy/Project/sages/pi-orchestrator');
  ...
  describe('TypeScript-specific detection', () => {
    ...
    it('should detect framework from package.json', async () => {
      ...
      // The pi-orchestrator package uses various patterns
      expect(Array.isArray(info.frameworks)).toBe(true);
    });
  });
});
